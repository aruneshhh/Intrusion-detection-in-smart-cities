...
label0 = Label(root, text="INTRUSION DETECTION SYSTEM USING YOLOV7", bg='light blue', font=('Times 23 bold'))
label0.pack(side=TOP)

label1 = Label(text="TYPE OF ZONE ", bg='light blue', font=('helvetica 15 bold'))
label1.place(x=60, y=100)

classes = ["NO PERSON ENTRY ZONE", "NO VEHICLE ENTRY ZONE", "COMPLETE INTRUSION FREE ZONE"]
combo1 = Combobox(root, values=classes, height=1, width=30)
combo1.place(x=68, y=130)

label2 = Label(text="Mode of operation", bg='light blue', font=('helvetica 15 bold'))
label2.place(x=350, y=100)

camera = ["webcam",  "offline video"]
combo2 = Combobox(root, values=camera, height=1, width=22)
combo2.place(x=355, y=130)

labelr = Label(text="Save", bg='light blue', font=('helvetica 15 bold'))
labelr.place(x=50, y=200)

v=StringVar(root,False)
style=Style(root)
style.configure("TRadiobutton",background ='light blue',foreground = 'light blue',font=("arial",20,"bold"))
values={"yes" :True,"no" :False}
Radiobutton(root,text="yes",variable=v,value =True).place(x=50,y=230)
Radiobutton(root,text="no",variable=v,value =False).place(x=95,y=230)

labelsf = Label(text="Show fps", bg='light blue', font=('helvetica 15 bold'))
labelsf.place(x=210, y=200)

sf=StringVar(root,False)
values={"yes" :True,"no" :False}
Radiobutton(root,text="yes",variable=sf,value =True).place(x=205,y=230)
Radiobutton(root,text="no",variable=sf,value =False).place(x=245,y=230)
...
def draw_boxes(img, bbox, identities=None, categories=None, confidences=None, names=None, colors=None,ip_address=None):
    #single frame
    global mailcount,mailthreshold,initialframe,initialobjects
    for i, box in enumerate(bbox):
        ...
        if not opt.nolabel :#and names[cat] == intrusion_type:
            ...
            if (point_inside_polygon(a, b, intfreezonebbox)) and (id not in initialobjects or initialframe):
                ...
                if mailcount:
                #    mailcount=time.time()
                #if  time.time()-mailcount>mailthreshold:
                    image = Img.fromarray(img.astype('uint8')).convert('RGB')
                    image.save('intrusion.jpg')
                    sendgmail('thiyagarajansethu2002@gmail.com')
                    mailcount=False
                #    print(time.time()-mailcount,"mailchecking")
                #    mailcount=time.time()
...
def camera():
    ...
    if combo1.get()=="NO PERSON ENTRY ZONE":
        intrusion_type=0
    if combo1.get()=="NO VEHICLE ENTRY ZONE":
    ...
    if (combo2.get() == "webcam"):
        ip_address = 0
        print(sf.get(),v.get())
        but3 = Button(root, padx=5, pady=5, width=42, bg='white', fg='black', relief=GROOVE,
                      command=lambda: main(ip_address, intrusion_type,v.get(),bool(int(sf.get()))), text='Run Intrusion Detection',
                      font=('helvetica 15 bold'))
        but3.place(x=55, y=450)
        ...
        but2 = Button(root, width=5, bg='white', fg='black', relief=GROOVE, command=get_ip, text='Enter',
                      font=('helvetica 8 bold'))
        but2.place(x=400, y=400)


but1 = Button(root, width=7, bg='white', fg='black', relief=GROOVE, command=camera, text='Enter',
              font=('helvetica 9 bold'))
but1.place(x=450, y=228)
...
but5 = Button(root, padx=5, pady=5, width=42, bg='white', fg='black', relief=GROOVE, text='EXIT', command=exitt,
              font=('helvetica 15 bold'))
but5.place(x=55, y=560)


root.mainloop()


    text = MIMEText("""<!DOCTYPE html>
...
<head>
	...
	<img src="kdb.jpg" alt="" width="300" style="height:auto;display:block;" />
</td>
		</tr>
		<tr>
